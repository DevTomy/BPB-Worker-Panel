...
async function buildXrayBestPingConfig(proxySettings, totalAddresses, chainProxy, outbounds, isFragment) {
    const remark = isFragment ? '♾️ Dino F - Best Ping 💥' : '♾️ Dino - Best Ping 💥';
...
async function buildXrayBestFragmentConfig(proxySettings, hostName, chainProxy, outbounds) {
    ...
    const config = buildXrayConfig(proxySettings, '♾️ Dino F - Best Fragment 😎', true, true, chainProxy, false, false);
...
async function buildXrayWorkerLessConfig(proxySettings) {
    const config = buildXrayConfig(proxySettings, '♾️ Dino F - WorkerLess ⭐', true, false, false, false, false);
...
export async function getXrayWarpConfigs (request, env, client) {
    ...
    for (const [index, endpoint] of warpEndpoints.split(',').entries()) {
        ...
        const warpConfig = buildXrayConfig(proxySettings, `♾️ DinoWorker - Warp${proIndicator}🇮🇷`, false, false, false, false, true);
        const WoWConfig = buildXrayConfig(proxySettings, `♾️ DinoWorker - WoW${proIndicator}🌍`, false, false, true, false, true);
